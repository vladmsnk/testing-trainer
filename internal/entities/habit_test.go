...
func TestGetCurrentPeriod(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			frequencyType:   Daily,
			startTrackingAt: currentTime.AddDate(0, 0, -3), // 3 days ago
			expected:        3,
		},
		{
			...
			expected:        0,
			startTrackingAt: currentTime.AddDate(0, 0, -1),
		},
		{
			name:            "Weekly - 1 week since creation",
			frequencyType:   Weekly,
			expected:        1,
			startTrackingAt: currentTime.AddDate(0, 0, -8),
		},
		{
			name:            "Weekly - 2 weeks since creation",
			frequencyType:   Weekly,
			startTrackingAt: currentTime.AddDate(0, 0, -14), // 2 weeks ago
			expected:        2,
		},
		{
			name:            "Monthly - 5 months since creation",
			frequencyType:   Monthly,
			startTrackingAt: currentTime.AddDate(0, -5, 0), // 5 months ago
			expected:        4,
		},
		{
			name:            "Monthly - 1 year and 2 months since creation",
			frequencyType:   Monthly,
			startTrackingAt: currentTime.AddDate(-1, -2, 0), // 1 year and 2 months ago
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := goal.GetCurrentPeriod(currentTime)
